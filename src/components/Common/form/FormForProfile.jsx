...
import { renderFieldInputForPost, renderFieldTextareaForPost } from "../units/renderField";
  ...
  validators.reduce((error, validator) => error || validator(value), undefined)
    const aboutMe=()=>{
      return <div className={s.aboutMeDesc}>
        <Field validate={composeValidators(required, maxLength(200))}  component={renderFieldTextareaForPost} placeholder="write some about yourself..." name="aboutMe"/>
      </div>
    }
      ...
      return <div className={s.contacts}>
        <ul>
          {Object.keys(props.profile.contacts).map(key=>{
            return <li key={key}>{key} - <Field  name={key} component={renderFieldInputForPost} placeholder="http://site.com"/></li>
          })}
        </ul>
      </div>
    }
  ...
  (
    <form onSubmit={handleSubmit}>
      <div className={s.fullnameContainer +" "+s.commonContainer}>
              <div >Полное имя</div>
              <div>
                <Field validate={composeValidators(required, maxLength(20))} component={renderFieldInputForPost} name="fullName"/>
              </div>
            </div>
